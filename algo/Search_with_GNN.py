...
class CodeGNN(nn.Module):
    ...
    def get_graph_embedding(self, g):
        ...
        g.edata['K'] = self.K(torch.cat((g.ndata['kind'][srcs],g.ndata['type'][srcs], edge_embds), dim=1))
        g.edata['V'] = self.V(torch.cat((g.ndata['kind'][srcs],g.ndata['type'][srcs], edge_embds), dim=1))
        ...
        g.ndata['h'] = self.W(torch.cat((g.ndata['h_n'], g.ndata['kind'], g.ndata['type']), 1))
        g.ndata['h'] = self.layernorm(g.ndata['h'])

        # second round
        g.ndata['Q'] = self.Q2(torch.cat((g.ndata['kind'], g.ndata['type'], g.ndata['h']), dim=1))
        g.edata['K'] = self.K2(torch.cat((g.ndata['kind'][srcs], g.ndata['type'][srcs], edge_embds, g.ndata['h'][srcs]), dim=1))
        g.edata['V'] = self.V2(torch.cat((g.ndata['kind'][srcs], g.ndata['type'][srcs], edge_embds, g.ndata['h'][srcs]), dim=1))
        ...
        g.ndata['h1'] = self.W2(torch.cat((g.ndata['h_n1'], g.ndata['h'], g.ndata['kind'], g.ndata['type']), 1))
