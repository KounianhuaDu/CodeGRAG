...
def build_instruction(src_language: str, dst_language: str, question: str, graph):
    return '''
...
The syntax graph of the given {} code is:\n
{}\n
You can refer it to do the translation.\n
The given problem:\n
{}
'''.strip().format(src_language, dst_language, src_language, graph, question.strip())
...
def generate_one_completion(problem, trans):
    ...
    if trans == 'cpp2python':
        task_id = problem['task_id'][4:] + '.pkl'
        graph_file = '../data/humaneval_cpp_graphs'
    elif trans == 'python2cpp':
        task_id = problem['task_id'][7:] + '.pkl'
        graph_file = '../data/humaneval_python_graphs/dgl'
        
    if task_id in os.listdir(graph_file):
        with open(os.path.join(graph_file, task_id), 'rb') as f:
            graph = pkl.load(f)
            graph = str(graph)
    else:
        graph = 'None.'
        
    prompt = build_instruction(src_language, dst_language, task, graph)
