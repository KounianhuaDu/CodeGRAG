...
parser.add_argument(
    "--model_path",
    type=str,
    default="/ext0/hcchai/codemate/llama2/Llama-2-13b-chat-hf",
)
...
def get_template(description):
    inputs = f"Below is an OJ programming problem, which is too long, I want you to simplify and summarize the problem to make the description shoter, the problem:\n {description}. ONLY give me the simplified problem and nothing else."
    # user_prompt = (
...
for question in tqdm(descriptions):
    try:
        answers.append(question2answer(model, tokenizer, get_template(question)))
    except:
        answers.append(descriptions)
datadealer.save_as_json(
    datadealer.extraction_fileter(descriptions, answers),
    os.path.join(
        f"../data/train/{args.dataset}", f"{args.language}_{args.chunk_interval}.json"
    ),
)
