...
parser.add_argument(
    ...
    "--trained_model_path",
    type=str,
    default="/home/jzchen/ML/Code/trained_models/CodeContest/c++/graphragextracted_Llama2/checkpoint-41",
)
...
adapters_weights = safetensors.safe_open(
    os.path.join(args.trained_model_path, "model.safetensors"), framework="pt"
)
# **** Important!! Must load in this way!!!****
state_dict = {
    k: adapters_weights.get_tensor(k).cuda()
...
for task_id, problem in datadealer.iter_test_data():
    ...
    try:
        generate_ids = model.llama_model.generate(
            tokenizer(
