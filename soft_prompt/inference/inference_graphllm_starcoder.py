...
def generate_and_tokenize_prompt(prompt, extracted_query):
    query = extracted_query if args.extracted_query else prompt
    task = extracted_query if args.extracted_prompt else prompt
    ...
    user_prompt = f"Question: Please use {args.language} to write a correct solution to a programming problem. You should give executable completed code and nothing else. We also have the syntax graph embedding of a similary problem encoded in <GraphEmb> for you to refer to.\nThe problem:\n{task}\n\nAnswer:"
    output_prompt = ""
...
problem_file = json.load(
    open(
        f"/home/jzchen/ML/AfterCodegrag/data/train/{args.dataset}/{args.language}_Gemma_test.json",
        "r",
    )
)
idx = 0
for task_id, problem in datadealer.iter_test_data():
    ...
    extracted_query = problem_file[idx]
    input_dict = generate_and_tokenize_prompt(prompt, extracted_query)
    ...
    try:
        generate_ids = model.starcoder_model.generate(
            tokenizer(
...
datadealer.save_results(
    samples,
    f"/home/jzchen/ML/AfterCodegrag/output/{args.language}graphrag{datadealer.extracted_token}_{args.extracted_prompt}_{args.model_path.split('/')[-1]}_{args.retrieval_method}.jsonl",
)
