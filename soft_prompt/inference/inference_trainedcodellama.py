...
parser.add_argument(
    "--model_path",
    type=str,
    default="/ext0/jzchen/model_weights/AI-ModelScope/CodeLlama-7b-Instruct-hf",
)
...
datadealer = DataDealer(
    dataset=args.dataset, split="test", extracted=False, softprompt=False
)
...
def generate_and_tokenize_prompt(task):
    ...
    user_prompt = f"<s>[INST] <<SYS>> You are a helpful coding assistant, and you should help to write programm correctly and efficiently. <</SYS>>. Please use {LANG} to write a correct solution to a programming problem. You should give executable completed code following the given input and output format. The problem:\n{task} [/INST]"
    output_prompt = ""
...
datadealer.save_results(
    samples,
    f"/home/jzchen/ML/Code/output/{args.model_path.split('/')[-1]}_{args.dataset}{args.load_trained_model}.jsonl",
)
