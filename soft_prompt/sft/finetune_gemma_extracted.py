...
datadealer = DataDealer(
    dataset=args.dataset, split="train", extracted=True, softprompt=False
)
args.output_path = f"../trained_models/{args.dataset}/{args.language}/{args.model_path.split('/')[-1]}_extractedsft/"
...
def generate_and_tokenize_prompt(data_point):
    ...
    user_prompt = f"<bos><start_of_turn>user\nPlease use python to write a correct solution to a programming problem. You should give executable completed code and nothing else.\nThe problem:\n{data_point['query']}<end_of_turn>\n"
    output_prompt = f"<start_of_turn>model\n{data_point['output']}<end_of_turn><eos>"
