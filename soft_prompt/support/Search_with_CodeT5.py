...
def search_with_faiss(query, data_list, index, pca, k):
    # 这个函数编码key，并在检索池里拿到相近的k个向量
    inputs = tokenizer.encode(query, return_tensors="pt", max_length=1024).to(device)
    ...
    emb = torch.tensor([data_list[idx.item()] for idx in indices[0][:k]])
